...
// Get conversations
$conversations = getUserConversations($buyer_id);

// Get messages for selected conversation
$messages = [];
if ($selected_user_id > 0) {
    $messages = getConversation($buyer_id, $selected_user_id);
    markMessagesAsRead($selected_user_id, $buyer_id);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
    ...
    if (!empty($message) && $selected_user_id > 0) {
        sendMessage($buyer_id, $selected_user_id, $message);
                    ...
                    <div class="messages-container">
                        <?php if (!empty($messages)): ?>
                            <?php foreach ($messages as $msg): ?>
                                <div class="message <?php echo $msg['sender_id'] == $buyer_id ? 'sent' : 'received'; ?>">
